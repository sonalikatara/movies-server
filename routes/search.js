...
router.get('/',function(req,res,next){
  let query =  req.query.query ? req.query.query : "popular";
  let page =  req.query.page ? req.query.page : 1;
  let reqUrl = API_URL_SEARCH_MOVIES + '&page='+ page+'&query='+query;
  axios.get(reqUrl).then(response => res.send(response.data.results.map(movie => ({...movie, rating : Math.round(movie.vote_average/2 )})))).catch(err => {
